<?php
    ...
    $option = 5; // A, B, C, D, E
?>
<h3 class="page-title">Ubah Latihan UN</h3>
<div class="row">
    <div class="col-md-12">
                ...
                <?php echo form_open_multipart("#", array('id' => 'form', 'class' => 'form-horizontal')); ?>
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-4">
                                <textarea id="description" name="description" class="form-control" rows="5"><?php echo isset($data['description']) ? $data['description'] : ''; ?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="datatable_ajax">
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="Tr<?php echo $row; ?>" role="row" class="filter">
                                            <td>
                                                <input id="QuizDetailID<?php echo $row; ?>" name="detail[<?php echo $row; ?>][id]" data-row="<?php echo $row; ?>" value="<?php echo $detail['id']; ?>" type="hidden" class="form-control form-filter input-sm">
                                            ...
                                            <td>
                                                <textarea id="QuizDetailQuestion<?php echo $row; ?>" name="detail[<?php echo $row; ?>][question]" class="form-control QuizDetailQuestion" rows="5"><?php echo $detail['question']; ?></textarea>
                                                <div class="mt-radio-list">
                                                    <?php
                                                        for($o=0; $o<$option; $o++){
                                                            echo '<label class="mt-radio mt-radio-outline">
                                                                    <input id="QuizDetailCorrectAnswer'.$row.'" name="detail['.$row.'][correct_answer]" value="'.$o.'" class="form-control QuizDetailCorrectAnswer" type="radio" '.(($detail['correct_answer'] == $detail['answer'][$o]['answer']) ? 'checked' : '').'>
                                                                    <textarea id="QuizDetailAnswer'.$row.'_'.$o.'" name="detail['.$row.'][answer]['.$o.']" class="form-control QuizDetailAnswer" rows="1">'.$detail['answer'][$o]['answer'].'</textarea>
                                                                    <span></span>
                                                                </label>';
                                                        }
                                                    ?>
                                            ...
                                            <td>
                                                <div class="col-md-12">
                                                    <select id="QuizDetailMaterialID<?php echo $row; ?>" name="detail[<?php echo $row; ?>][material_id]" class="form-control QuizDetailMaterialID">
                                                        <?php echo getMaterialBySubject($subject_id,$detail['material_id']); ?>
                                                    </select>
                                                </div>
                                                <div class="col-md-12">
                                                    <br/><textarea id="QuizDetailSolution<?php echo $row; ?>" name="detail[<?php echo $row; ?>][solution]" class="form-control QuizDetailSolution" rows="5"><?php echo $detail['solution']; ?></textarea>
                                                </div>                                                
                                            </td>
                                            <td>
            ...
            submitHandler: function () {
                ...
                for(i=0; i<=rows; i++){
                    if($('#QuizDetailQuestion'+i).val()!=undefined){
                        if($('#QuizDetailID'+i).val()!=undefined){
                            post_data.append('detail['+i+'][id]',$('#QuizDetailID'+i).val());
                        }
                ...
                $.ajax({
                  type: 'POST',
                  url: '<?php echo base_url()."subject/".$subject_id."/quiz/update/".$data['id']; ?>',
                  data: post_data,
                  processData: false,
                  contentType: false,
                  success: function(data){
                    if(data != 0) {
                      swal({
                        title: '<h4>Latihan UN Gagal Diedit!</h4>',
                        text: data,
                      ...
                      swal({
                        title: '<h4>Latihan UN Berhasil Diedit!</h4>',
                        text: 'Selamat, latihan UN berhasil diedit.',
